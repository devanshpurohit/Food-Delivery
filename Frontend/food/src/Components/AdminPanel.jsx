...
const AdminDashboard = () => {
  ...
  const handleStatusChange = (name, status) => {
    setRestaurantStatus((prev) => ({ ...prev, [name]: status }));
  };
  ...
  const renderDashboard = () => (
    <div className="w-full space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        {[ 
          { title: "Customers", value: "3,782", color: "green", change: "+11.01%" },
          { title: "Delivery Boys", value: "128", color: "green", change: "+3.20%" },
          { title: "Restaurants", value: "45", color: "green", change: "+5.67%" },
        ].map((stat, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow flex flex-col">
            ...
            <p className={`mt-2 text-sm text-${stat.color}-600`}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow">
        <h3 className="text-xl font-semibold mb-1 text-black dark:text-white">Monthly Target</h3>
        ...
        <div className="flex flex-col items-center">
          <div className="relative w-40 h-20">
            <svg className="w-full h-full" viewBox="0 0 200 100">
              <path d="M10 90 A90 90 0 0 1 190 90" fill="none" stroke="#e5e7eb" strokeWidth="15" />
              <path d="M10 90 A90 90 0 0 1 160 40" fill="none" stroke="#6366f1" strokeWidth="15" strokeLinecap="round" />
            </svg>
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[10%] text-center">
              <p className="text-2xl font-bold text-black dark:text-white">85.55%</p>
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 w-full text-center">
            <div>
              ...
              <p className="font-bold text-black dark:text-white">$20K <span className="text-red-500">↓</span></p>
            </div>
            <div>
              <p className="text-gray-400 text-sm">Revenue</p>
              <p className="font-bold text-black dark:text-white">$20K <span className="text-green-500">↑</span></p>
            </div>
            <div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow">
        <h3 className="text-xl font-semibold mb-4 text-black dark:text-white">Monthly Sales</h3>
        <Bar
          data={{
            ...
            datasets: [
              {
                label: "Sales",
                data: [150, 370, 190, 280, 170, 180, 170, 120, 200, 360, 270, 100],
  ...
  const renderRestaurants = () => {
    const filtered = restaurantDetails.filter(
      (r) => restaurantFilter === "all" || restaurantStatus[r.name] === restaurantFilter
    );

    return (
      <div className="w-full">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold text-gray-800 dark:text-white">All Restaurants</h2>
          <button
            onClick={() => setDarkMode(!darkMode)}
            className="p-2 rounded-full bg-gray-200 dark:bg-gray-700 text-black dark:text-white"
          >
          ...
          <table className="min-w-full bg-white dark:bg-gray-800 rounded-lg shadow">
            ...
            <tbody>
              {filtered.map((rest) => (
                ...
                >
                  <td className="px-6 py-4 text-black dark:text-white">{rest.name}</td>
                  <td className="px-6 py-4 text-black dark:text-white">{rest.owner}</td>
                  <td className="px-6 py-4 text-black dark:text-white">{rest.address}</td>
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-medium ${
                        restaurantStatus[rest.name] === "pending"
                          ? "bg-yellow-100 text-yellow-800"
                          : restaurantStatus[rest.name] === "active"
                          ? "bg-green-100 text-green-800"
                          : "bg-red-100 text-red-800"
                      }`}
                    >
                      {restaurantStatus[rest.name]}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <div className="flex gap-2 justify-center">
                      <button
                        onClick={() => handleStatusChange(rest.name, "active")}
                        className="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 text-sm"
                      >
                        Active
                      </button>
                      <button
                        onClick={() => handleStatusChange(rest.name, "removed")}
